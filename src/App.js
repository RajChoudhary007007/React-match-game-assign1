...
class App extends Component {
  ...
  matchingImageId = id => {
    const cardItem = imagesList.find(eachImage => eachImage.id === id)

    console.log(cardItem)

    this.setState(prevState => ({
      clickedCardList: [...prevState.clickedCardList, cardItem.id],
    }))
  }
  ...
  getRandomImage = () => {
    ...
    const randomImage = Math.floor(Math.random() * activeImage.length)
    return randomImage
  }
    ...
    return (
      <>
        <img
          ...
          src={activeImage[randomImage].imageUrl}
          alt="category"
        />
      </>
    )
